...
double MotorShaftPower::calculate() {
    if (loadEstimationMethod_ == FieldData::LoadEstimationMethod::POWER) {
        ...
        while (true) {
            MotorCurrent motorCurrent(motorRatedPower_, motorRPM_, efficiencyClass_, tempLoadFraction_, ratedVoltage_);
            current = motorCurrent.calculate();
            MotorEfficiency motorEfficiency(motorRPM_, efficiencyClass_, motorRatedPower_, tempLoadFraction_);
        ...
        tempLoadFraction_ -= 0.01;
        MotorCurrent motorCurrent1(motorRatedPower_, motorRPM_, efficiencyClass_, tempLoadFraction_, ratedVoltage_);
