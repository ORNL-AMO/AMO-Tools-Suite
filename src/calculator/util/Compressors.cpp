...
CompressorsBase::OutputBlowOff Compressors_Centrifugal_BlowOff::calculateFromVIPFMeasured_BlowOff(double V, double I, double PF, double blowPer) {
    return Compressors_Centrifugal_BlowOff::calculateFromPerkW_BlowOff(V * I * PF * 1.732 / 1000, blowPer);
}
...
CompressorsBase::Output Compressors_Centrifugal_LoadUnload::calculateFromVIPFMeasured(double V, double I, double PF) {
    return Compressors_Centrifugal_LoadUnload::calculateFromPerkW(V * I * PF * 1.732 / 1000);
}
...
CompressorsBase::Output Compressors_Centrifugal_ModulationUnload::calculateFromVIPFMeasured(double V, double I, double PF) {
    return Compressors_Centrifugal_ModulationUnload::calculateFromPerkW(V * I * PF * 1.732 / 1000);
}
...
CompressorsBase::Output Compressors_ModulationWOUnload::calculateFromVIPFMeasured(double V, double I, double PF) {
    return Compressors_ModulationWOUnload::calculateFromPerkW(V * I * PF * 1.732 / 1000);
}
...
CompressorsBase::Output Compressors_StartStop::calculateFromVIPFMeasured(double V, double I, double PF) {
    return Compressors_StartStop::calculateFromPerkW(V * I * PF * 1.732 / 1000);
}
...
double Compressors_LoadUnload::CurveFit(double value, bool capacityVPower) const {
    ...
    const double kW_ul = (kW_max - kW_maxmod) * pow(C_ul / C_fl, x_mod) + kW_maxmod;
    ...
    do {
        ...
        if(C_ul != C_fl) {
            if(C_curve < C_ul){
                ...
                kW_avg_mod = (kW_max - kW_maxmod) * pow((P_max + P_mod - P_avg_mod) / P_avg_mod, x_mod) + kW_maxmod;
            }
            else {
                kW_avg_mod = (kW_max - kW_maxmod) * pow(C_curve / C_fl, x_mod) + kW_maxmod;
            }
        }
        ...
        if (C_curve < C_ul) {
            ...
            kW_curve = (t_bd * kW_avg_bd + t_ol * kW_avg_ol + t_rl * kW_avg_rl + t_rpu * kW_avg_rpu + ((C_ul != C_fl) ? t_rmod * kW_avg_mod : 0)) /
                       (t_bd + t_ol + t_rl + t_rpu + (C_ul != C_fl ? t_rmod : 0));
        }
...
CompressorsBase::Output Compressors_LoadUnload::calculateFromPerkW(double PerkW) {
    ...
    if(CPer*100 >= PerC_ul && (CntrlType == ControlType::ModulationUnload || CntrlType == ControlType::VariableDisplacementUnload)){
        return Compressors_ModulationWOUnload(kW_fl, C_fl, kW_nl).calculateFromPerkW(PerkW);
    }
...
CompressorsBase::Output Compressors_LoadUnload::calculateFromPerC(double CPer) {
    ...
    if(CPer >= PerC_ul && (CntrlType == ControlType::ModulationUnload || CntrlType == ControlType::VariableDisplacementUnload)){
        return Compressors_ModulationWOUnload(kW_fl, C_fl, kW_nl).calculateFromPerC(CPer);
    }
...
CompressorsBase::Output Compressors_LoadUnload::calculateFromVIPFMeasured(double V, double I, double PF) {
    return Compressors_LoadUnload::calculateFromPerkW(V * I * PF * 1.732 / 1000);
}
