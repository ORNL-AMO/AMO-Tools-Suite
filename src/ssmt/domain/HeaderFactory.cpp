...
const Header HeaderFactory::make(const double &headerPressure, const Boiler &boiler) const {
    const std::string methodName = std::string("HeaderFactory::") + std::string(__func__) + ": ";
...
const Header HeaderFactory::make(const std::shared_ptr<HeaderNotHighestPressure> &mediumPressureHeaderInput,
    ...
    if (isUseTurbine) {
        std::cout << methodName << "isUseTurbine=true, adding highToMediumPressureTurbine" << std::endl;
...
const Header HeaderFactory::make(const double headerCountInput,
    ...
    if (isUseTurbineHighToLow) {
        std::cout << methodName << "isUseTurbine=true, adding highToLowPressureTurbine" << std::endl;
    ...
    if (headerCountInput == 3 && isUseTurbineMediumToLow) {
        std::cout << methodName << "isUseTurbine=true, adding mediumToLowPressureTurbine" << std::endl;
    ...
    if (isFlashCondensate) {
        ...
        if (headerCountInput == 3) {
            std::cout << methodName << "isFlashCondensate=true & 3 headers, adding mediumPressureCondensateFlashTank"
                      << std::endl;
            ...
            std::cout << methodName << "isFlashCondensate=true & not 3 headers, adding highPressureCondensateFlashTank"
                      << std::endl;
    ...
    if (isBlowdownFlashed) {
        std::cout << methodName << "isBlowdownFlashed=true, adding blowdownFlashTank" << std::endl;
...
HeaderFactory::make(const double headerPressure, const SteamSystemModelerTool::FluidProperties &returnCondensate,
    ...
    if (isUseTurbine) {
        ...
        const Inlet &condensingTurbineInlet = inletFactory.make(condenserPressure, condensingTurbine);
