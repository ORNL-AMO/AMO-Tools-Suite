...
NAN_METHOD(WasteWaterTreatment)
{
    ...
    catch (std::runtime_error const &e)
    {
        ...
        ThrowError(std::string("std::runtime_error thrown in WasteWaterTreatment Output - wasteWater.h: " + what).c_str());
    }
    info.GetReturnValue().Set(r);
}

NAN_METHOD(WasteWaterTreatmentGivenSRT)
{
    inp = Nan::To<Object>(info[0]).ToLocalChecked();
    r = Nan::New<Object>();
    ...
    const double MicrobialDecay = getDouble("MicrobialDecay", inp);
    const double MaxUtilizationRate = getDouble("MaxUtilizationRate", inp);
    //max days and time increment unused in calculation without iteration (DefinedSRT)
    const double MaxDays = 0;
    const double TimeIncrement = 0;
    const double OperatingDO = getDouble("OperatingDO", inp);
    const double Alpha = getDouble("Alpha", inp);
    const double Beta = getDouble("Beta", inp);
    const double SOTR = getDouble("SOTR", inp);
    ...
    const double Speed = getDouble("Speed", inp);
    const double EnergyCostUnit = getDouble("EnergyCostUnit", inp);
    const double DefinedSRT = getDouble("DefinedSRT", inp);
    try
    {
        auto wwTreatment = WasteWater_Treatment(Temperature,
                                                So,
                                                ...
                                                EnergyCostUnit,
                                                DefinedSRT);
        WasteWater_Treatment::OutputWithoutTable output = wwTreatment.calculateGivenSRT();
    ...
    {
        std::string const what = e.what();
        ThrowError(std::string("std::runtime_error thrown in WasteWaterTreatment OutputWithoutTable - wasteWater.h: " + what).c_str());
    }
    info.GetReturnValue().Set(r);
}
